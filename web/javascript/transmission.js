...
Transmission.prototype =
{
	...
	updateStats: function( stats )
	{
		...
		setInnerHTML( $('#stats_session_ratio')[0], Math.ratio(session["uploadedBytes"],session["downloadedBytes"]).toRatioString());
		...
		setInnerHTML( $('#stats_total_ratio')[0], Math.ratio(total["uploadedBytes"],total["downloadedBytes"]).toRatioString());
	...
	updateInspector: function()
	{
		...
		setInnerHTML( tab.availability, torrents.length ? Math.ratio( total_availability*100, sizeWhenDone ).toPercentString() + '%' : na );
		setInnerHTML( tab.ratio, torrents.length ? Math.ratio( total_upload, total_download ).toRatioString() : na );
		...
		setInnerHTML( tab.progress, torrents.length ? Math.ratio( sizeDone*100, sizeWhenDone ).toPercentString() + '%' : na );
